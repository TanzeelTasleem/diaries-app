...
import { createDiaryEntry, GET_ENTRIES } from "../../features/entrySlice/entrySlice";
...
const date =(date : any )=>{
  ...
  return (
    <div>
      <Header />
      <Box>
      <EntryModal open={open} handleClose={handleClose} handleSubmit={handleSubmit} />
        ...
        <Box display="flex" justifyContent="space-evenly" flexWrap="wrap"> 
           {
            ...
            (
                entries.map((entry ,index)=>{
                    return(
                        <Box minWidth="300px" m={5} key={index} boxShadow={5} p={3} display="flex" flexDirection="column" borderRadius={10}>
                            <Typography className={classes.title} variant="h5">{entry.title}</Typography>
                            <Typography variant="subtitle1">{entry.content}</Typography>
                            <Box mt={1} display="flex" justifyContent="space-between">
                                  <Typography variant="subtitle2">Created at: {date(entry.createdAt)}</Typography>
                                   <Box pl={5}></Box>
                                  <Typography variant="subtitle2">Updated at:{date(entry.updatedAt)}</Typography>
                            </Box>
                        </Box>
                    )
                })
